...
- 外部から自プロセスがフォアグラウンドになると `focus.rs` の `EVENT_SYSTEM_FOREGROUND` フックで検出し、`update()` でZ順序を更新する（winit が視覚的なZ順序を反映しない問題への対処）。ただし「マウスホバーでフォーカス（アクティブウィンドウ追跡）かつ前面に移動しない」設定の場合はユーザー意図を尊重して持ち上げない。判定は `should_raise_on_foreground()`（`SPI_GETACTIVEWINDOWTRACKING` が有効なら `SPI_GETACTIVEWNDTRKZORDER` を参照）で行う。設定取得失敗時は安全側で従来動作（持ち上げる）にフォールバック。フックは自ウィンドウの HWND も `MAIN_HWND` に記録する（外向きD&D用）
- 外向きD&D（`drag_drop::start_drag` → `DoDragDrop`）はマウスをキャプチャするが、キャプチャできるのはフォアグラウンドウィンドウのみ（MSDN: "Only the foreground window can capture the mouse"）。フォーカス追従（前面に移動しない）設定ではドラッグ開始時に自ウィンドウがフォアグラウンドでない場合があり、その状態だとキャプチャに失敗してドラッグできない（一度クリックして明示的にアクティブ化する必要があった）。対策として `DoDragDrop` 直前に `focus::main_hwnd()` で取得したHWNDを `SetForegroundWindow` する。ホバー時ではなくドラッグ開始（明示操作）時のみ持ち上げるため、前面に移動しない設定の意図は維持される
