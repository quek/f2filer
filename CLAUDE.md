...
- 外部から自プロセスがフォアグラウンドになると `focus.rs` の `EVENT_SYSTEM_FOREGROUND` フックで検出し、`update()` でZ順序を更新する（winit が視覚的なZ順序を反映しない問題への対処）。ただし「マウスホバーでフォーカス（アクティブウィンドウ追跡）かつ前面に移動しない」設定の場合はユーザー意図を尊重して持ち上げない。判定は `should_raise_on_foreground()`（`SPI_GETACTIVEWINDOWTRACKING` が有効なら `SPI_GETACTIVEWNDTRKZORDER` を参照）で行う。設定取得失敗時は安全側で従来動作（持ち上げる）にフォールバック。
- 外向きD&D（`drag_drop::start_drag` → `DoDragDrop`）はマウスをキャプチャするが、キャプチャできるのはフォアグラウンド**スレッド**のみ（MSDN: "Only the foreground window can capture the mouse"）。フォーカス追従（前面に移動しない）設定ではドラッグ開始時に自ウィンドウがフォアグラウンドでない場合があり、その状態だとキャプチャに失敗してドラッグできない。対策として `DoDragDrop` 直前に `ensure_foreground()` で自ウィンドウを `SetForegroundWindow` し、`GetForegroundWindow()` のスレッドIDが自スレッドと一致するか**事後確認**する。`SetForegroundWindow` はフォアグラウンドロック（他プロセスが直前に入力を受けていた等）で黙って失敗するため、不一致なら `AttachThreadInput` で現フォアグラウンドスレッドに入力キューを一時接続して再試行する。HWND はフックが記録したものではなく `eframe::Frame` の raw window handle から取る（`drag_drop::window_hwnd()`）——フックは winit 補助ウィンドウがフォアグラウンドになった場合そちらを記録してしまう。`DoDragDrop` が 100ms 未満で戻った場合はユーザー操作ではありえないので開始失敗と判断し、hr・経過時間・フォアグラウンド状態をステータスバーに赤字で出す（再現時の診断用）。ホバー時ではなくドラッグ開始（明示操作）時のみ持ち上げるため、前面に移動しない設定の意図は維持される
- winit は画面 `(0,0)` に無題・可視の補助ウィンドウ `"Winit Thread Event Target"`（リサイズ中の WM_PAINT 受信用）を生成する。マウスホバーでフォーカス環境で外部から `WinActivate "ahk_exe f2filer.exe"` するとこの**無題ウィンドウを掴むことがあり**、OS がアクティブ化先ウィンドウ中心へカーソルを移すため、カーソルが画面左上 `(8,8)` へ飛ぶ。**f2filer 側でこの補助ウィンドウを `SW_HIDE`/移動して潰してはいけない**——`WinWait` が掴んだ直後に消えて `WinActivate` が `"Target window not found"` で破綻する。正しい対処は AHK 側でタイトル名指し（`WinWait "f2filer ahk_exe f2filer.exe"`）でメイン（class `Window Class`, title `f2filer`）を掴ませること。カーソルがアクティブ化先へ移るのは OS の正しい挙動なので f2filer 側の修正は不要
...
### 1. コードレビュー
コード変更が完了しビルドが通った後、コミット前に必ず全変更箇所をレビューする。
- 変更した全ファイルを読み、以下の観点でチェック:
  - **セキュリティ**: インジェクション（コマンド、SQL、パス）、未検証の外部入力、権限チェック漏れ
  - **正確性**: エッジケース（空リスト、特殊文字、境界値）、エラーハンドリングの漏れ
  - **DRY**: 重複コードが新たに発生していないか
  - **設計意図との整合**: CLAUDE.md の Design Decisions / Coding Principles に違反していないか
- 問題を発見したら修正してから再ビルド → 再レビュー
- レビュー結果をユーザーに報告し、指摘事項があればその内容と修正内容を伝える
